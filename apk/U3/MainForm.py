...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        self.actionRandomTerrain = QtGui.QAction(parent=MainWindow)
        self.actionRandomTerrain.setCheckable(True)
        self.actionRandomTerrain.setChecked(True)
        self.actionRandomTerrain.setText("Random terrain")
        self.actionHillTerrain = QtGui.QAction(parent=MainWindow)
        self.actionHillTerrain.setCheckable(True)
        self.actionHillTerrain.setText("Hill terrain")
        self.actionValleyTerrain = QtGui.QAction(parent=MainWindow)
        self.actionValleyTerrain.setCheckable(True)
        self.actionValleyTerrain.setText("Valley terrain")

        self.menuSettings.addAction(self.actionRandomTerrain)
        self.menuSettings.addAction(self.actionHillTerrain)
        self.menuSettings.addAction(self.actionValleyTerrain)
        ...
        self.actionDT.triggered.connect(lambda: self.Canvas.setViewDT(self.actionDT.isChecked()))
        self.actionContour_lines_2.triggered.connect(lambda: self.Canvas.setViewContours(self.actionContour_lines_2.isChecked()))
        self.actionSlope.triggered.connect(lambda: self.Canvas.setViewSlope(self.actionSlope.isChecked()))
        self.actionExposition.triggered.connect(lambda: self.Canvas.setViewExposition(self.actionExposition.isChecked()))
        self.actionOpen.triggered.connect(self.openClick)
        self.actionRandomTerrain.triggered.connect(lambda: self.setTerrainMode("random"))
        self.actionHillTerrain.triggered.connect(lambda: self.setTerrainMode("hill"))
        self.actionValleyTerrain.triggered.connect(lambda: self.setTerrainMode("valley"))

        self.retranslateUi(MainWindow)
        
    def openClick(self):
        #Open dialog to select a file
        res = QFileDialog.getOpenFileName(self.Canvas, "Open", "", "XYZ Files (*.xyz);;All Files (*)")
        ...
        try:
            ...
            for line in f:
                ...
                parts = line.split()
                #Check if we have at least X, Y, Z
                if len(parts) >= 3:
            ...
            #Get available drawing area with 50px margins
            W = self.Canvas.width() - 100
            H = self.Canvas.height() - 100
            
            #Calculate total width and height of data
            rx = max_x - min_x
            ry = max_y - min_y
            
            #Clear old points from canvas
            self.Canvas.getPoints().clear()
            
            #Recalculate each point to fit the screen
            for p in points:
                #Normalization and scaling to pixels
                nx = 50 + (p.x() - min_x) / rx * W
                ny = 50 + (max_y - p.y()) / ry * H
                #Save scaled point back to canvas
                self.Canvas.getPoints().append(QPoint3DF(nx, ny, p.z()))
    ...
    def analyzeSlopeClick(self):
        ...
        #Set data and show slope
        self.Canvas.setTriangles(triangles)
        #Switch on slope analysis mode in canvas
        self.Canvas.setViewSlope(True)
        #Update the menu checkbox to show as active
        self.actionSlope.setChecked(True)
        #Refresh the screen to show changes
        self.Canvas.repaint()

    def analyzeExpositionClick(self):
        #Get current triangulation from canvas
        DT = self.Canvas.getDT()

        #If DT does not exist, create it first
        if not DT:
            self.createDTClick()
            DT = self.Canvas.getDT()
        ...
        a = Algorithms()
        triangles = []
        # Run the aspect analysis
        a.analyzeDTMAspect(DT, triangles)
        
        #Pass calculated triangles to canvas
        self.Canvas.setTriangles(triangles)
        #Enable aspect view mode
        self.Canvas.setViewExposition(True)
    ...
    def setTerrainMode(self, mode):
        self.Canvas.setTerrainMode(mode)

        self.actionRandomTerrain.setChecked(mode == "random")
        self.actionHillTerrain.setChecked(mode == "hill")
        self.actionValleyTerrain.setChecked(mode == "valley")
